                        ...
                        <div class="new-user-info">
                            <div class="row">
                                ...
                                <div class="form-group col-md-4">
                                    ...
                                    <select name="document_id" class="form-control select2js" id="document_id" required>
                                        <option
                                            value="">{{ __('message.select_field', ['name' => __('message.document')]) }}</option>
                                        @foreach ($documents as $document)
                                            <option
                                                value="{{ $document->id }}" {{isset($id) && $document->id == $data->document_id ? ' selected' : ''}}>{{ $document->getTranslation('name',app()->getLocale()) }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                {{--                                <div class="form-group col-md-4">--}}
                                ...
                                <div class="form-group col-md-4">
                                    <label class="form-control-label" for="expire_date">{{ __('message.expire_date') }}
                                        <span class="text-danger"
                                              id="has_expiry_date">{{ $has_expiry_date == 1 ? '*' : ''  }}</span>
                                    </label>
                                ...
                                <div class="form-group col-md-4">
                                    <label class="form-control-label"
                                           for="driver_document">{{ __('message.upload_document') }} <span
                                            class="text-danger" id="document_required"></span> </label>
                                    <div class="custom-file">
                                        <input type="file" id="driver_document" name="driver_document"
                                ...
                                @if( isset($id) && getMediaFileExit($data, 'driver_document'))
                                    <div class="col-md-2 mb-2">
                                            ...
                                            ?>
                                        @if($extention)
                                            <img id="driver_document_preview" src="{{ $image }}" alt="#"
                                                 class="attachment-image mt-1">
                                        @else
                                            <img id="driver_document_preview" src="{{ asset('images/file.png') }}"
                                                 class="attachment-file">
                                        @endif
                                        <a class="text-danger remove-file"
                                           href="{{ route('remove.file', ['id' => $data->id, 'type' => 'driver_document']) }}"
