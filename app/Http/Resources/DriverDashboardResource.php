...
class DriverDashboardResource extends JsonResource
{
    ...
    {
            $on_ride_request = RideRequest::whereNotIn('status', ['canceled'])->where('is_driver_rated', false)->first();
        
        $pending_payment_ride_request = RideRequest::where('status', 'completed')->where('is_driver_rated',true)
                        ->whereHas('payment',function ($q) {
                            $q->where('payment_status', 'pending');
                        })
                        ->first();
        // dd($this->driverRideRequestDetail);
